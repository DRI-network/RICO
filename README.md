...
![](https://i.gyazo.com/0300b95fed0b436322212e26a7f9280b)

### Initialize
```
     init(totalSupply, tobAmountToken, tobAmountWei, PoDCapToken, poDCapWei, _projectOwner);
```
- _projectOwner ( address ) -- a projcetOwner is a params of responsible token manager in RICO concept.if Ethereum address is checksumed, it may unmatch address that unable checksumed while the EVM operate codes.
...
### Add Token Distribution Round 
```
     addRound(Supply, SupplyTime, _to);
```
- Supply (uint256) 
